 ...
 * the exact body size.
 */
export const PAD_BUCKETS = [1024, 4096, 16384, 65536] as const;
export const PAD_SCHEME_ISO7816 = "iso7816-4" as const;
...
export function pickBucket(plaintextLen: number): number {
  ...
  throw new Error(
    `[etzhayyim-sdk/crypto] plaintext ${plaintextLen} bytes exceeds largest inline ` +
 ...
 */
export function padIso7816(plain: Uint8Array, targetLen: number): Uint8Array {
  if (plain.length + 1 > targetLen) {
    throw new Error(
      `[etzhayyim-sdk/crypto] padding target ${targetLen} too small for ` +
...
export function unpadIso7816(padded: Uint8Array): Uint8Array {
...
export function encrypt(opts: EncryptOpts): EncryptedEnvelope {
  ...
  let padScheme: PadScheme | undefined;
  ...
  if (pad !== "none") {
    ...
    if (pad === "bucket") {
      ...
      if (pad.bucket <= AEAD_TAG_BYTES + 1) {
        throw new Error(
          `[etzhayyim-sdk/crypto] explicit bucket ${pad.bucket} too small`
        );
      }
...
export function decrypt<T = unknown>(opts: DecryptOpts): T {
  ...
  const plaintextBytes =
    envelope.pad === PAD_SCHEME_ISO7816 ? unpadIso7816(padded) : padded;
