...
}): { session: SessionHandle; handshake: HybridKemHandshake } => {
  const info = new TextEncoder().encode(
    `${args.senderDid}|${args.recipientDid}`,
  );
...
export const establishSessionResponder = (args: {
  ...
  const key = hybridDecapsulate(
    args.handshake,
    args.recipientKemSecret,
    args.recipientKemPublic,
    info,
  );
  const sessionId = `session_${Buffer.from(randomBytes(16)).toString('hex')}`;
  sessions.set(sessionId, {
