...
export const PQ_SUITE = "pqh-v1" as const;
...
function combineSharedSecrets(args: {
  ...
  return hkdf(
    sha256,
    concatBytes(args.ssClassical, args.ssPq),
    KEM_COMBINER_SALT,
    info,
...
export function hybridEncapsulate(
  recipient: HybridKemPublicBundle,
  info?: Uint8Array
): HybridEncapsulation {
  ...
  const {cipherText, sharedSecret: ssPq} = ml_kem768.encapsulate(
...
export function hybridDecapsulate(
  handshake: HybridKemHandshake,
  secret: HybridKemSecretBundle,
  recipientPublic: HybridKemPublicBundle,
  info?: Uint8Array
): Uint8Array {
  assertSuite(handshake.suite);
  assertSuite(secret.suite);
  const ssClassical = x25519.getSharedSecret(
